...
namespace ando {
	namespace memory {
		...
		public:
			bool writeByte(::std::uint64_t address, BYTE data);
			bool writeInt(::std::uint64_t address, int data);
			bool writeFloat(::std::uint64_t address, float data);
			bool writeLong(::std::uint64_t address, long data);
			bool writeString(::std::uint64_t address, ::std::string data);

			template <typename T>
			bool writeVector2(::std::uint64_t address, const math::Vector2<T> data) {
				return this->write(address, data.getXData(), (2 + 1) * sizeof(T));
			}

			template <typename T>
			bool writeVector3(::std::uint64_t address, const math::Vector3<T> data) {
				return this->write(address, data.getXData(), (3 + 1) * sizeof(T));
			}

			template <typename T>
			bool writeVector4(::std::uint64_t address, const math::Vector4<T> data) {
				return this->write(address, data.getXData(), (4 + 1) * sizeof(T));
			}

			template <typename T, ::std::size_t width, ::std::size_t height>
			bool writeMatrix(::std::uint64_t address, const math::Matrix<T, width, height> data) {
				return this->write(address, data.getData(), width * height * sizeof(T));
			}

			template <typename T>
			bool write(::std::uint64_t address, const T *data, ::std::size_t size = sizeof(T)) {
