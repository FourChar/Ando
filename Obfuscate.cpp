...
void real_AddNops(unsigned char *start, unsigned char *end)
{
	int iLeft = end - start;
	while (iLeft) {
		if (GenRand(0, 7) < 5) {
			...
			end[-(iLeft--)] = (unsigned char)(iSize & 0xFF);
			while (iSize) {
				end[-iLeft] = (unsigned char)(GenRand(0, 256) & 0xFF);
				iSize--;
				iLeft--;
			}
		}
...
void __stdcall _AddNops(unsigned char *start, unsigned char *end) {
	DWORD dwOld, dwOld2;
...
unsigned char *__stdcall AllocateJumpgate(unsigned char *target, int minlen, int maxlen) {
	PBYTE buf;
	DWORD dwOld;
...
void __stdcall FreeJumpgate(unsigned char *target) {
